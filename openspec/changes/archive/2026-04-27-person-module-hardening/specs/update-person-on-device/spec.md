...
async updatePersonOnDevice(person: PersonRecord): Promise<void> {
  await this.put('/ISAPI/AccessControl/UserInfo/Record', {
    employeeNo: person.employee_id,
    name: person.name,
    // ... other fields
  })
  // NO card assignment call!
}
```
...
### Scenario 4: Card removed from person
```
Given a person with employeeNo="EMP001", cardNumber="123456" on device
And DB has card_number="" (card removed)
When updatePersonOnDevice is called
Then PUT /ISAPI/AccessControl/UserInfo/Record is called
And if adapter supports card unassign → call it, else leave as-is
```

### Scenario 5: Card already exists on different user
```
...
The sync loop passes `previousCardNumber` to detect changes:
```typescript
const previousCardNumber = existingPerson.card_number
await adapter.updatePersonOnDevice(updatedPerson, previousCardNumber)
```
