...
### Card Change Detection in Adapter
The adapter's `updatePersonOnDevice` must compare incoming `cardNumber` with current device value to determine if `assignCardToDevice` is needed. See `update-person-on-device/spec.md` for full details.
