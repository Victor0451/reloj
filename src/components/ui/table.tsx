...
function TableRow({
  ...
  variant = "default",
  ...props
}: React.ComponentProps<"tr"> & { variant?: "default" | "zebra" }) {
  return (
    <tr
      data-slot="table-row"
      data-variant={variant}
      className={cn(
        "border-b transition-colors duration-150 hover:bg-muted/50 data-[variant=zebra]:bg-muted/30 data-[state=selected]:bg-muted",
        className
      )}
      {...props}
    />
